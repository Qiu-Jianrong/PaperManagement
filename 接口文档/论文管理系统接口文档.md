...
> 请求方式：DELETE
>
> 接口描述：该接口用于根据ID删除文献库，用户只能删除自己的文献库。文献库中的文章亦会被**级联删除**！需提醒用户将文章移走，慎用！
...
### 2.6 文献库检索(未定可扩展，可以仿照bing搜索引擎，增加搜索建议)
...
| abstract   | 论文摘要     | string | 是       |                |      |
| content    | 论文正文     | string | 是       |                |      |
| state      | 发布状态     | string | 是       | 已发布 \| 草稿 |      |
| categoryId | 文献库ID   | number | 是       |                |      |
